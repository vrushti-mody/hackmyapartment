const MIN_ROOM_IMAGE_OPTIONS = 10;

function buildQueryOptions(query: string, count: number): string[] {
  return Array.from({ length: count }, (_, index) => {
    const sig = index + 1;
    return `https://source.unsplash.com/1080x1920/?${encodeURIComponent(query)}&sig=${sig}`;
  });
}

function ensureRoomImageOptions(curated: string[], query: string): string[] {
  const generated = buildQueryOptions(query, MIN_ROOM_IMAGE_OPTIONS);
  const merged = Array.from(new Set([...curated, ...generated]));
  return merged.slice(0, Math.max(MIN_ROOM_IMAGE_OPTIONS, curated.length));
}

const ROOM_FALLBACK_IMAGES: Record<string, string[]> = {
  "living room": ensureRoomImageOptions(
    [
  ...
  bedroom: ensureRoomImageOptions(
    [
  ...
  kitchen: ensureRoomImageOptions(
    [
  ...
  bathroom: ensureRoomImageOptions(
    [
  ...
  outdoor: ensureRoomImageOptions(
    [
